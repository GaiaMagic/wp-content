			...
			if ( !is_array($mostpopular) || empty($mostpopular) ) {
				...
				foreach ($mostpopular as $wppost) {					
					...
					if ( $instance['stats_tag']['comment_count'] ) {
						...
						$post_stats .= "<a href='".get_permalink($the_ID)."' class=\"comment-count r\">" . $comment_count . "</a>";
					}
					...
					if ( $instance['stats_tag']['author'] ) {
						if ($post_stats != "") {
							$post_stats .= "<div class=\"author\">".$wppost->display_name."</div>";
						} else {					
							$post_stats .= "<div class=\"author\">".$wppost->display_name."</div>";
						}
					}
					if ( $instance['stats_tag']['date']['active'] ) {
						if ($post_stats != "") {
							$post_stats .= "<time class=\"time\">".date_i18n($instance['stats_tag']['date']['format'], strtotime($wppost->date_gmt))."</time>";
						} else {					
							$post_stats .= "<time class=\"time\">".date_i18n($instance['stats_tag']['date']['format'], strtotime($wppost->date_gmt))."</time>";
						}
					}
					...
					if (!empty($post_stats)) {
						$stats = ' <div class="meta l">' . $post_stats . '</div> ';
					}
					...
					$data = array(
						'title' => '<a href="'.get_permalink($the_ID).'" title="'. $title_attr . '" class="title l"><span class="wpp-post-title">'. $tit .'</span></a>',
					...
					if ($instance['markup']['custom_html']) {
						if ($instance['markup']['pattern']['active']) {
							...
							$content .= htmlspecialchars_decode($instance['markup']['post-start'], ENT_QUOTES) . $thumb . $stats . '<a href="'.get_permalink($the_ID).'" title="'. $title_attr .'" class="title l"><span class="wpp-post-title">'. $tit .'</span></a>'.$post_content.' ' . $rating . htmlspecialchars_decode($instance['markup']['post-end'], ENT_QUOTES) . "\n";
						}
					} else {
						$content .= '<li>'. $thumb . $stats . '<a href="'. get_permalink($the_ID) .'" title="'. $title_attr .'" class="title l"><span class="wpp-post-title">'. $tit .'</span></a>'. $post_content .' '.  $rating .'</li>' . "\n";
					}
				}			
